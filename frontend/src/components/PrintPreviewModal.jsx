...
import { rankStrengthEntries } from '../utils/strengthRanking';
...
const PrintPreviewModal = ({ open, onClose, schedule, participants, results, user, isTeamRanking = false }) => {
  ...
  const strengthRankedParticipants = isStrengthPrint ? rankStrengthEntries(participants, {
    getParticipant: (participant) => participant,
    getScore: (participant) => getScoreData(participant).finalScore,
    isAbsent: (participant) => getScoreData(participant).isAbsent,
    getEvents: (participant) => results[participant.__printKey || participant._id || participant]?.details?.events
  }) : null;
  ...
  const sortedParticipants = (() => {
    ...
    if (isStrengthPrint) return strengthRankedParticipants.map(({ entry }) => entry);
  ...
  const participantRanks = isDivingPrint ? divingRankedParticipants.map(({ rank }) => rank) : (isStrengthPrint ? strengthRankedParticipants.map(({ rank }) => rank) : getRanks(sortedParticipants));
  ...
  const renderStrengthDetailTable = () => {
    ...
    return <TableContainer><Table size="small" className="diving-detail-table" sx={{ tableLayout: 'fixed', width: '100%', '& .MuiTableHead-root .MuiTableCell-root': { border: '1px solid black', padding: '3px 2px', color: 'black', fontSize: '9.5pt', fontFamily: '"SimSun", "宋体", serif' }, '& .MuiTableBody-root .MuiTableCell-root': { border: 'none', padding: '3px 2px', color: 'black', fontSize: '9.5pt', fontFamily: '"SimSun", "宋体", serif' } }}><TableHead><TableRow>{headings.map((heading) => <TableCell key={heading} align="center" sx={{ fontWeight: 'bold' }}>{heading}</TableCell>)}</TableRow></TableHead><TableBody>{sortedParticipants.flatMap((participant, participantIndex) => { const result = results[participant.__printKey || participant._id || participant]; const { finalScore, isAbsent } = getScoreData(participant); const events = result?.details?.events || []; const count = Math.max(1, events.length); const name = participant?.name || participant?.user?.name || '未知'; const unit = participant?.schoolName || participant?.teamName || '-'; const participantRows = Array.from({ length: count }, (_, eventIndex) => { const event = events[eventIndex]; return <TableRow key={`${participant._id || participantIndex}-${eventIndex}`}>{eventIndex === 0 && <TableCell rowSpan={count} align="center" className="detail-name-cell">{name}</TableCell>}{eventIndex === 0 && <TableCell rowSpan={count} align="center" className="detail-unit-cell">{unit}</TableCell>}<TableCell align="center" className="detail-action-cell">{event ? normalizeStrengthActionName(event.actionName) : (isAbsent ? '弃权' : '-')}</TableCell><TableCell align="center">{event?.rawScore ?? '-'}</TableCell><TableCell align="center">{event?.points ?? '-'}</TableCell>{eventIndex === 0 && <TableCell rowSpan={count} align="center">{isAbsent ? '弃权' : finalScore}</TableCell>}{eventIndex === 0 && <TableCell rowSpan={count} align="center">{participantRanks[participantIndex]}</TableCell>}{eventIndex === 0 && <TableCell rowSpan={count} align="center">{isAbsent || participantRanks[participantIndex] === 1 ? '' : (leaderScore - finalScore).toFixed(2)}</TableCell>}</TableRow>; }); if (participantIndex < sortedParticipants.length - 1) participantRows.push(<TableRow key={`${participant._id || participantIndex}-spacer`} sx={{ height: 14 }}><TableCell colSpan={headings.length} sx={{ border: 'none !important', p: '0 !important', fontSize: 0 }} /></TableRow>); return participantRows; })}</TableBody></Table></TableContainer>;
  };
      ...
      <DialogContent dividers>
        ...
        <Box sx={{ mb: 4, p: 2, bgcolor: '#f5f5f5', borderRadius: 1 }} className="no-print">
          <Grid container spacing={2}>
            ...
            {(isDivingPrint || isStrengthPrint) && isAdmin && <Grid item xs={12}>
              <Typography variant="body2" sx={{ mb: 1 }}>{isStrengthPrint ? '素质力量裁判员姓名（将显示在公告页尾）' : '跳水裁判员姓名（将显示在公告页尾）'}</Typography>
        ...
        >
          ...
          {/* Table */}
          {isStrengthPrint ? (isStrengthDetailPrint ? renderStrengthDetailTable() : renderDivingRankTable()) : isDivingPrint ? (isDivingDetailPrint ? renderDivingDetailTable() : renderDivingRankTable()) : <TableContainer sx={{ border: '2px solid black' }}>
          ...
          {(isDivingPrint || isStrengthPrint) && <Box sx={{ mt: 3, display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '4px 24px', fontSize: '12pt', color: 'black', fontFamily: '"SimSun", "宋体", serif', lineHeight: 1.8 }}>
