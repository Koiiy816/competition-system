...
const getParticipantDivingProgram = (participant) => {
  ...
  if (plan && Array.isArray(plan.dives) && plan.dives.length) {
    return plan.dives.map((dive) => ({
      ...
      difficulty: dive?.difficulty === '' || dive?.difficulty == null ? (/陆上|陸上/.test(String(participant?.event || '')) ? 1 : dive?.difficulty) : dive.difficulty,
...
const getStrengthEvents = (participant, schedule) => {
  ...
  return configured.map((action, index) => ({
    actionCode: action.actionCode || '', actionName: String(action.actionName || action.actionCode || `小项${index + 1}`)
